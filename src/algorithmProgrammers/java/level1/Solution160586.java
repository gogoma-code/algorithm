	...
	public int[] solution2(String[] keymap, String[] targets) {
		...
		for(String map : keymap) {
			...
			for(int i=0; i<keys.length; i++) {
				...
				if(alphabet[idx] == 0) alphabet[idx] = i+1;
				else alphabet[idx] = i < alphabet[idx] ? (i+1) : alphabet[idx]; 
			}
		}
...
//		sol.solution2(new String[] {"ABACD", "BCEFD"}, new String[] {"ABCD","AABB"});
		sol.solution2(new String[] {"AA"}, new String[] {"B"});
//		sol.solution2(new String[] {"AGZ",  "BSSS"}, new String[] {"ASA", "BGZ"});
	}
}
